import { Feature, GenderType, Toilet, ToiletType } from "../models/toilet";
import {
  calculateRealisticRating,
  generateRealisticReviews,
} from "../utils/reviewHelpers";

const generateToiletData = (): Toilet[] => {
  const baseToilets = [
    {
      id: 1,
      ...
      reviews: [
        {
          ...
          rating: 4,
          comment:
            "Clean facilities with good amenities. Located on 3rd floor.",
        },
        {
          id: 502,
          userName: "Derek M.",
  ...
  return baseToilets.map((toilet) => {
    const reviews = generateRealisticReviews(toilet.id);
    const rating = calculateRealisticRating(reviews);
...
export const toiletsData: Toilet[] = generateToiletData();
